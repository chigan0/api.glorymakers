...
class RestorPassword(Resource):
	...
	def post(self):
		...
		if re != None and re.login == data['login'] and re.confirmed == True:
			...
			esn(render_template,app,text='Confirm Your Email',name=data['login'],email=data['email'],token=token,url='http://127.0.0.1:5000/v1/login/restore_password/')
			return jsonify({'message':'confrim your email'})

		return jsonify({'message':'dadawdwa'})
