...
class Users(db.Model):
	...
	rating = db.Column(db.String,default=0) #Users Rating default 0
	completed_orders = db.relationship('OrderCompleted',backref='order',lazy='dynamic')
...
#Выполненый заказ ползователя
class OrderCompleted(db.Model):
	__tablename__='order'
	id = db.Column(db.Integer,primary_key=True,unique=True,nullable=False)
	user_public_id = db.Column(db.String,db.ForeignKey('user.public_id'))
	public_order_id = db.Column(db.String,unique=True,nullable=False)
